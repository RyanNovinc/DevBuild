// src/screens/CommunityScreen/WhyJoinTabRevamped.js
...
const WhyJoinTabRevamped = ({ theme }) => {
  const [selectedBenefit, setSelectedBenefit] = useState(null);
  const [expandedDomain, setExpandedDomain] = useState(null);
  const fadeAnim = useRef(new Animated.Value(0)).current;
  const scaleAnim = useRef(new Animated.Value(0.95)).current;
  
  // Individual animation values for each domain card
  const domainAnimations = useRef(
    Array(8).fill(null).map(() => ({
      scale: new Animated.Value(1),
      opacity: new Animated.Value(1),
      height: new Animated.Value(80), // Default card height
    }))
  ).current;
  ...
  const handleDomainPress = (index) => {
    const isExpanding = expandedDomain !== index;
    
    if (expandedDomain !== null && expandedDomain !== index) {
      // Collapse the currently expanded domain first
      Animated.parallel([
        Animated.timing(domainAnimations[expandedDomain].height, {
          toValue: 80,
          duration: 250,
          useNativeDriver: false,
        }),
        Animated.spring(domainAnimations[expandedDomain].scale, {
          toValue: 1,
          tension: 100,
          friction: 8,
          useNativeDriver: true,
    ...
    if (isExpanding) {
      ...
      Animated.parallel([
        Animated.timing(domainAnimations[index].height, {
          toValue: 180, // Expanded height
          duration: 300,
          useNativeDriver: false,
        }),
        Animated.spring(domainAnimations[index].scale, {
          toValue: 1.02,
          tension: 100,
          friction: 8,
          useNativeDriver: true,
            ...
            {benefits.map((benefit) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected && (
                    <Animated.View style={styles.benefitDetail}>
                ...
                <Animated.View
                  ...
                  style={[
                    styles.domainCard,
                    {
                      ...
                      height: domainHeights[index],
                      transform: [{ scale: domainAnimations[index].scale }],
                    }
                  ]}
                >
                  <TouchableOpacity
                    onPress={() => handleDomainPress(index)}
                    activeOpacity={0.7}
                    style={styles.domainTouchable}
                  >
                    ...
                    {isExpanded && (
                      <Animated.View style={styles.channelsList}>
...
export default WhyJoinTabRevamped;