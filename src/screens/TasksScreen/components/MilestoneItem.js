// src/screens/TasksScreen/components/MilestoneItem.js
...
const MilestoneItem = ({ 
  ...
  const milestoneTasks = getTasksForMilestone(item.id);
  
  // Wrapper function that also tracks goal progress updates
  const handleStatusChangeWithTracking = (milestoneId, newStatus) => {
    console.log(`🎯 [UI DEBUG] handleStatusChangeWithTracking called - Milestone: ${milestoneId}, Status: ${newStatus}`);
    // First call the original handler
    handleChangeMilestoneStatus(milestoneId, newStatus);
  ...
  const getStatusData = () => {
    if (item.status === 'todo' || (!item.status && item.progress === 0)) {
      return {
        color: '#E0E0E0',
        text: 'To Do',
        accessibilityLabel: 'Milestone status: To Do'
      };
    } else if (item.status === 'done' || (!item.status && item.progress === 100)) {
      return {
        color: '#81C784',
        text: 'Done',
        accessibilityLabel: 'Milestone status: Done'
      };
    } else {
      return {
        color: '#64B5F6',
        text: 'In Progress',
        accessibilityLabel: 'Milestone status: In Progress'
      };
    }
  };
  ...
  // Create style for milestone item
  ...
  return (
    <Animated.View
      ...
      ref={ref => {
        if (ref && !milestoneLayout[item.id]) {
          ref.measure((x, y, width, height, pageX, pageY) => {
            setMilestoneLayout(prev => ({
              ...prev,
              [item.id]: {
                id: item.id,
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('MilestoneDetails', { milestoneId: item.id, mode: 'edit' })}
        onLongPress={() => {
          // Show milestone actions menu on long press
          showMilestoneActionsMenu(item);
        }}
        delayLongPress={500}
        activeOpacity={0.7}
        accessible={true}
        ...
        accessibilityHint={`Tap to view milestone details, hold to show actions`}
      >
        <View 
          style={[
        ...
        {/* Show action buttons for milestone status change */}
          ...
          {currentStatus === 'todo' && (
            <>
              ...
              <TouchableOpacity 
                ...
                accessibilityHint="Changes milestone status to In Progress"
              >
                <Ionicons 
                  name="arrow-forward" 
                  size={scaleWidth(18)} 
              ...
              <TouchableOpacity 
                style={[styles.actionButton, { minWidth: 44, minHeight: 44 }]}
                onPress={() => {
                  console.log(`🚀 TICK BUTTON CLICKED FOR MILESTONE: ${item.title} (${item.id}) 🚀`);
                  console.log(`🚀 STARTING MILESTONE COMPLETION FLOW 🚀`);
                  handleStatusChangeWithTracking(item.id, 'done');
                }}
                accessible={true}
                accessibilityRole="button"
                accessibilityLabel="Mark as Done"
                accessibilityHint="Changes milestone status to Done"
              >
                <Ionicons 
          ...
          {currentStatus === 'in_progress' && (
            <>
              ...
              <TouchableOpacity 
                ...
                accessibilityHint="Changes milestone status to To Do"
              >
                <Ionicons 
                  name="arrow-back" 
                  size={scaleWidth(18)} 
