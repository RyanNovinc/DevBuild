// src/screens/TasksScreen/components/MilestonesList.js
...
// Store animation values for milestone items
const milestoneAnimations = {};

const MilestonesList = ({ taskScreenProps }) => {
  const {
    theme,
    navigation,
    isDarkMode,
    ...
    viewMode, // Current view mode ('milestones' or 'tasks')
    milestones, // Need access to the milestones array
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (!draggingMilestone || !activeMilestoneSection) return;
        
        // Find the section milestones
        const sectionMilestones = sectionData
          .find(section => section.id === activeMilestoneSection)?.data || [];
        
        // Get layout for all milestones in this section
        const layoutsForSection = Object.values(milestoneLayout)
          .filter(layout => layout.sectionId === activeMilestoneSection)
          .sort((a, b) => a.y - b.y);
        
        // Find current position
        const draggedPosition = gestureState.dy + milestoneLayout[draggingMilestone.id]?.originalY || 0;
        ...
        layoutsForSection.forEach((layout, idx) => {
          if (layout.id === draggingMilestone.id) return;
      ...
      onPanResponderRelease: () => {
        ...
        setActiveMilestoneSection(null);
        
        // Reset all items to original position
        Object.values(milestoneLayout).forEach(layout => {
          layout.ref?.setNativeProps({
            style: { transform: [{ translateY: 0 }] }
          });
        });
  ...
  const renderItem = ({ item, section, index }) => {
    ...
    // If in milestones view, render milestone item
    if (viewMode === 'milestones') {
      // Determine if this is the dragged item
      const isBeingDragged = draggingMilestone && draggingMilestone.id === item.id;
      ...
      return (
        <MilestoneItem
          ...
          showMilestoneActionsMenu={showMilestoneActionsMenu}
          handleChangeMilestoneStatus={handleChangeMilestoneStatus}
          isBeingDragged={isBeingDragged}
          panY={panY}
          milestoneLayout={milestoneLayout}
          setMilestoneLayout={setMilestoneLayout}
          panHandlers={isBeingDragged ? panResponder.panHandlers : {}}
        />
      );
    } 
    // If in tasks view and this is a milestone section within a goal section
    else if (viewMode === 'tasks' && section.isGoal && item.isMilestone) {
      // If milestone section has no tasks, don't render it
      if (!item.data || item.data.length === 0) return null;
      
      // Check if this milestone is collapsed
      const isMilestoneCollapsed = collapsedMilestones[item.id] === true;
      
      // Get task count limit text
      const taskCountText = getTaskLimitText(item.data.length);
      
      // Render milestone header with tasks beneath it
      return (
        <View style={{
          ...
          {/* Milestone Header - Made collapsible with vibrant gradient background */}
          <TouchableOpacity
            style={{
              flexDirection: 'row',
              alignItems: 'center',
            ...
            onPress={() => toggleMilestoneCollapse(item.id)}
            activeOpacity={0.7}
            accessible={true}
            accessibilityRole="button"
            accessibilityLabel={`${item.title} milestone, ${isMilestoneCollapsed ? 'collapsed' : 'expanded'}`}
            accessibilityHint={`${isMilestoneCollapsed ? 'Expand' : 'Collapse'} to ${isMilestoneCollapsed ? 'show' : 'hide'} ${item.data.length} tasks`}
          >
            ...
            <View style={{
              backgroundColor: `${item.color}20`, // Semi-transparent background based on milestone color
              paddingHorizontal: 8,
              paddingVertical: 3,
              borderRadius: 12,
            ...
            <View style={{
              ...
              <Animated.View style={{
                transform: [{
                  rotate: isMilestoneCollapsed ? '0deg' : '180deg'
                }]
              }}>
                <Ionicons 
                  name="chevron-down" 
          ...
          {/* Tasks for this milestone - only show if not collapsed */}
                  ...
                  <View style={{
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => navigation.navigate('MilestoneDetails', {
                        milestoneId: item.id, // Navigate to the parent milestone
                        mode: 'edit',
                    ...
                    >
                      <Ionicons
                        name="pencil"
                        size={scaleWidth(16)}
                        color={item.color} // Use milestone color for consistency
                      />
                    </TouchableOpacity>
                  </View>
                </View>
  ...
  const renderSectionHeader = ({ section }) => {
    ...
    // Only render goal section headers (not milestone sections)
    if (viewMode === 'tasks' && !section.isGoal) {
      return null;
    }
    
    // Get milestone count for this goal section if we're in tasks view
    if (viewMode === 'tasks' && section.isGoal) {
      // Count milestones for this goal that are actually in the current data
      // This ensures we're counting the actual visible milestones in the section
      const milestonesForGoal = section.data.length;
      
      // For task view, modify the section to show milestone count with limit
      const milestoneLimit = FREE_PLAN_LIMITS?.MAX_MILESTONES_PER_GOAL || 2;
      
      // Create a modified section with updated milestone count
      const modifiedSection = {
        ...section,
        milestoneCount: milestonesForGoal,
    ...
    // Default rendering for milestones view
    return (
      <SectionHeader
        section={section}
        theme={theme}
  ...
  const getEmptyStateAccessibilityLabel = () => {
    if (viewMode === 'milestones') {
      if (selectedGoalId !== 'all') {
        return "No milestones linked to this goal yet. Create a milestone to get started.";
      }
      return "No milestones yet. Milestones help you organize your tasks and track progress towards your goals.";
    } else {
      if (selectedGoalId !== 'all') {
  ...
  const getEmptyStateMessage = () => {
    if (viewMode === 'milestones') {
      return selectedGoalId !== 'all'
        ? "No milestones linked to this goal yet"
        : "Milestones help you organize your tasks and track progress towards your goals";
    } else {
      return selectedGoalId !== 'all'
  ...
  return (
    <SectionList
      ...
      contentContainerStyle={[
        styles.milestonesList,
        sectionData.length === 0 ? { flex: 1 } : { paddingBottom: scaleHeight(100) }
      ]}
      ListEmptyComponent={
        <View style={[styles.emptyContainer]}>
          <CustomEmptyState
            title={viewMode === 'milestones' ? "No Milestones Yet" : "No Tasks Yet"}
            message={getEmptyStateMessage()}
            icon={viewMode === 'milestones' ? "folder-open" : "list-outline"}
          ...
          {/* Data Fix Option - Only show in milestones view */}
          {viewMode === 'milestones' && totalMilestoneCount !== visibleItemCount && (
            <TouchableOpacity 
              style={[
              ...
              accessible={true}
              accessibilityRole="button"
              accessibilityLabel="Fix milestone count"
              accessibilityHint="Fixes inconsistencies in milestone data"
            ...
            >
              ...
              >
                {isVerifying ? 'Checking Data...' : 'Fix Milestone Count'}
              </Text>
            </TouchableOpacity>
          )}
        </View>
      ...
      // Accessibility props
      accessible={true}
      accessibilityLabel={viewMode === 'milestones' ? "Milestones list" : "Tasks list"}
      accessibilityHint={viewMode === 'milestones' ? "Shows your milestones grouped by goals" : "Shows your tasks grouped by goals and milestones"}
    />
  );
};
