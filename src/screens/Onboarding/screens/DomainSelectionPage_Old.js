...
const DomainSelectionPage = ({ 
  ...
  const [wheelVisible, setWheelVisible] = useState(true); // Start with wheel visible (no typing animation)
  ...
  const [isWelcomeState, setIsWelcomeState] = useState(true); // Track welcome state from DomainWheel
  
  const selectedCountry = parentSelectedCountry;
  const setSelectedCountry = onCountrySelected;
  const [showCountryDropdown, setShowCountryDropdown] = useState(false);
  const [countryButtonShake, setCountryButtonShake] = useState(false);
  ...
  // Country button shake animation
  ...
  useEffect(() => {
    if (wheelVisible) {
      ...
      Animated.parallel([
        ...
        // Wheel is now ready for interaction - segments will reveal on centre button click
      });
    }
  }, [wheelVisible]);
  ...
  const revealSegmentsSequentially = () => {
    ...
    // Show centre button message and celebration immediately when reveal starts
    cycleToNextMessage();
    setShowMessage(true);
  ...
  useEffect(() => {
    if (selectedDomain && !showDomainInfo) {
      ...
      // Make sure centre button message is hidden when domain is selected
      if (showMessage) {
        Animated.parallel([
          Animated.timing(centerMessageOpacity, {
            toValue: 0,
  ...
  const handleDomainSelect = (domain) => {
    ...
    if (selectedDomain && selectedDomain.name === domain.name) {
      ...
      // Message and celebration already triggered on first centre tap, so no need here
  ...
  const handleCenterButtonPress = () => {
    ...
    // Only show celebration if segments are already revealed (subsequent centre button clicks)
    if (segmentsRevealed) {
  ...
  const triggerCountryShake = () => {
    setCountryButtonShake(true);
    Animated.sequence([
      Animated.timing(countryShakeAnim, {
        toValue: 10,
        duration: 100,
        useNativeDriver: true
      }),
  ...
  const handleContinue = () => {
    ...
    if (!selectedCountry) {
      // Trigger shake animation on country button
      triggerCountryShake();
  ...
  return (
    <View style={styles.container}>
      {/* Info button - always visible in top left */}
      <TouchableOpacity
        style={styles.fixedInfoButton}
      ...
      {/* Country selector - show after welcome state */}
      {!isWelcomeState && (
        <Animated.View style={{ transform: [{ translateX: countryShakeAnim }] }}>
          <TouchableOpacity
            style={[
              styles.fixedCountrySelector,
              !selectedCountry && styles.countryRequiredButton
            ]}
            onPress={() => setShowCountryDropdown(true)}
          >
            {selectedCountry ? (
              <>
                <ResponsiveText style={styles.countrySelectorText}>
                  {getAvailableCountries().find(c => c.code === selectedCountry)?.flag || '🌍'}
                </ResponsiveText>
                <ResponsiveText style={[styles.countrySelectorText, { fontSize: 11 }]}>
      ...
      {/* Skip button - only show after welcome state */}
      {!isWelcomeState && (
        <TouchableOpacity
          ...
          hitSlop={{ top: 15, bottom: 15, left: 15, right: 15 }}
        >
          <ResponsiveText style={styles.skipButtonText}>Skip</ResponsiveText>
        </TouchableOpacity>
      )}

      {/* Navigation header with title - only show when not in welcome state */}
      {!isWelcomeState && (
        <NavigationHeader 
          ...
          onBack={null} // No back button - this is the first screen
          leftComponent={null} // Info button now fixed position
          rightComponent={null} // Buttons now fixed position
        />
      )}
        ...
        {wheelVisible && (
          ...
          >
            <DomainWheel 
              ...
              onWelcomeStateChange={(welcomeState) => {
                setIsWelcomeState(welcomeState);
      ...
      {/* Clean Professional Country Selection Modal */}
      {showCountryDropdown && (
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.modalContainer}>
            ...
            {/* Countries Grid - 2 columns for better space usage */}
            <ScrollView 
            ...
            >
              {getAvailableCountries().map((country) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <ResponsiveText style={styles.countryEmoji}>
                    {country.flag}
                  </ResponsiveText>
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    ...
    paddingBottom: 40,
    paddingTop: 0, // No top padding to maximise space
  },
  fullScreenTouchable: {
  ...
  guidedHand: {
    position: 'absolute',
    top: 60, // Position closer to centre button
    left: '50%',
  ...
  fixedSkipButton: {
    position: 'absolute',
    top: 20, // Above the country selector
    right: 20, // Same right alignment as country selector
    paddingHorizontal: 16,
    paddingVertical: 8,
    borderRadius: 20,
    backgroundColor: 'rgba(255, 255, 255, 0.1)',
  ...
  countryRequiredButton: {
    backgroundColor: 'rgba(239, 68, 68, 0.9)', // Red background when required
    borderColor: '#EF4444',
    borderWidth: 2,
  },
