// src/screens/TodoListScreen/components/tabs/SimpleLaterTab.js
// Simplified LaterTab focused on keyboard functionality with original styling
...
/**
 * Simplified LaterTab with basic TextInput - no complex state management
 */
const SimpleLaterTab = ({
  todos,
  setTodos,
  tomorrowTodos,
  setTomorrowTodos,
    ...
    if (item.isGroup) {
      return (
        <TodoGroup
          ...
          onDelete={(id) => deleteTodo(id, 'later')}
    ...
    if (!item.groupId) {
      return (
        <TodoItem
          ...
          onDelete={() => deleteTodo(item.id, 'later')}
          onLongPress={showTodoOptions}
          theme={theme}
          fillCheckbox={true}
        />
...
export default React.memo(SimpleLaterTab);