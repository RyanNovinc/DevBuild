...
const TodoTab = (props) => {
  ...
  useEffect(() => {
    // Animate the content opacity for transitions
    Animated.timing(contentOpacity, {
      toValue: 0,
      duration: 150,
      useNativeDriver: true,
    }).start(() => {
      // After fade out completes, update the UI state
      Animated.timing(contentOpacity, {
        toValue: 1,
        duration: 250,
        useNativeDriver: true,
...
const TodoListScreen = () => {
  ...
  // Reset to Todo tab when component mounts
  useFocusEffect(
    React.useCallback(() => {
      setMainActiveTab('TODO');
      // Force tab navigator to remount with TODO as initial tab
  ...
  // Dismiss keyboard when navigating away from screen
  useEffect(() => {
