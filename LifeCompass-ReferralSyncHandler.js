...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (path) {
      ...
      case '/validate-referral-availability':
        if (method === 'POST') {
          return await handleValidateReferralAvailability(body, headers);
        }
        break;
...
async function handleValidateReferralCode(body, headers) {
  try {
    ...
    // Check if user has already SUCCESSFULLY used a referral code
    if (userResult.Item && userResult.Item.enteredReferralCode && userResult.Item.conversionRecorded) {
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ 
          success: false, 
          error: 'You have already successfully used a referral code. Only one working referral code can be used per account.' 
        })
      };
    }
...
async function handleSyncReferralCode(body, headers) {
  try {
    ...
    // Double-check if user already has a referral code (extra safety)
    const userResult = await docClient.send(new GetCommand({
      TableName: TABLES.USERS,
      Key: { deviceId: deviceId }
    }));
...
// Validate if a referral code can still accept conversions (simple availability check)
async function handleValidateReferralAvailability(body, headers) {
  try {
    ...
    // Check if code is still available (not used and active)
    ...
    return {
      ...
      body: JSON.stringify({ 
        available: available,
        message: available ? 'Referral code is available' : 'Referral code has already been used'
      })
    };

  } catch (error) {
    console.error('Error validating referral availability:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ 
        available: true, // Default to available on error to prevent false negatives
