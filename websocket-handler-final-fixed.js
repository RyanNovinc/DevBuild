    ...
    if (routeKey === '$connect') {
      ...
      if (message.action === 'sendMessage') {
        ...
        try {
          ...
          let shouldBreakStreaming = false; // Flag to break out of streaming loops

          // AGGRESSIVE Smart Buffering Decision - Catch function calls immediately
          const shouldBufferResponse = (toolCalls, accumulatedResponse) => {
            const hasToolCalls = toolCalls.length > 0;
            const responseLength = accumulatedResponse.length;
            
            console.log('🎯 AGGRESSIVE BUFFERING DECISION:', {
              hasToolCalls,
              responseLength,
              toolCallCount: toolCalls.length,
              decision: hasToolCalls ? 'BUFFER IMMEDIATELY (function calls detected - STOP STREAMING NOW)' : 'STREAM (conversational only)'
            });
            
            // AGGRESSIVE RULE: The MOMENT we detect ANY function calls, STOP streaming entirely
            // This prevents ANY data truncation in modals
            if (hasToolCalls) {
              console.log('🚨 FUNCTION CALLS DETECTED - SWITCHING TO FULL BUFFERING MODE');
              return true;
            }
            
            // RULE: Pure conversational responses (no function calls) can be streamed safely
            return false;
          };
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (content) {
                  ...
                  // AGGRESSIVE Smart Buffering: Check immediately for function calls
                  const shouldBuffer = shouldBufferResponse(toolCalls, accumulatedResponse);
                  
                  // AGGRESSIVE Buffering: The moment function calls are detected, STOP all streaming
                  if (!shouldBuffer) {
                    // Safe to stream - no function calls detected yet
                    await sendToClient(connectionId, {
                      type: 'chunk',
                      content,
                      conversationId,
                    ...
                    // FIRST TIME detecting function calls - IMMEDIATELY switch to buffering mode
                    console.log('🚨 FUNCTION CALLS DETECTED - IMMEDIATELY STOPPING STREAMING FOR DATA INTEGRITY');
                    await sendToClient(connectionId, {
                    ...
                    shouldBreakStreaming = true; // Set flag to break out of streaming loops
                  }
                  // If shouldBuffer && isExecutionPhase, accumulate silently (no streaming)
                }
              } catch (parseError) {
              ...
              // AGGRESSIVE: Break immediately if function calls detected
              if (shouldBreakStreaming) {
                console.log('🚨 BREAKING OUT OF STREAMING LOOP - FUNCTION CALLS DETECTED');
                break;
              }
            }
            
            // Break out of outer loop if execution phase detected
            if (shouldBreakStreaming) {
              console.log('🚨 BREAKING OUT OF OUTER STREAMING LOOP - SWITCHING TO FULL BUFFERING');
              break;
            }
          }

          // Use streaming function calls as-is - they should be complete by now
