// WebSocket Message Handler for LifeCompass - CORRECTED VERSION
// Based on original working file with ONLY smart buffering fix added
...
// Enhanced system prompt with updated standalone guidance (ORIGINAL WORKING VERSION)
...
- CALCULATE dates from today (${new Date().toISOString().split('T')[0]}) when timeframes specified
- ENSURE data consistency between related items`;

// Define tools (function calling schemas) with ORIGINAL WORKING DEFINITIONS
const tools = [
  {
    type: "function",
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          targetDate: {
            type: "string",
            description: "ISO date string for target completion if user specified a timeframe. Calculate from today's date (${new Date().toISOString().split('T')[0]}). Examples: '6 months' = '2026-03-05', '3 months' = '2025-12-05'. Use null if no timeframe mentioned."
          }
        },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          dueDate: {
            type: "string",
            description: "ISO date string for due date if user specified timeframes. Calculate from today's date (${new Date().toISOString().split('T')[0]}) based on user's plan. Examples: 'in 1.5 months' = '2025-10-20', 'by December' = '2025-12-01'. Use null if no timeframe mentioned."
          },
          tasks: {
            type: "array",
            items: {
    ...
    if (routeKey === '$connect') {
      ...
      if (message.action === 'sendMessage') {
        ...
        try {
          ...
          // SMART BUFFERING: Always enable streaming initially
          // Decision on buffering vs streaming happens during processing
          ...
          // SMART BUFFERING DECISION - Simple and Reliable (THE ONLY CHANGE FROM ORIGINAL)
          const shouldBufferResponse = (toolCalls) => {
            const hasToolCalls = toolCalls.length > 0;
            
            console.log('🎯 SMART BUFFERING DECISION:', {
              hasToolCalls,
              toolCallCount: toolCalls.length,
              decision: hasToolCalls ? 'BUFFER (function calls detected)' : 'STREAM (conversational only)'
            });
            
            // SIMPLE RULE: If there are function calls, always buffer for data integrity
            // This prevents modal data corruption and ensures complete data
            if (hasToolCalls) {
              return true;
            }
            
            // SIMPLE RULE: Pure conversational responses can be streamed safely
            // This maintains good UX for planning/explanation phases
            return false;
          };

          // Process the stream (ORIGINAL STREAMING LOGIC)
            ...
            for (const line of lines) {
              ...
              try {
                ...
                if (content) {
                  ...
                  // SMART BUFFERING: Check if we should buffer for data integrity
                  const shouldBuffer = shouldBufferResponse(toolCalls);
                  
                  // Smart Buffering: Stream content if safe, buffer if function calls detected
                  if (!shouldBuffer) {
                    await sendToClient(connectionId, {
                      type: 'chunk',
                      content,
                  ...
                  } else if (!isExecutionPhase && shouldBuffer) {
                  ...
                  // If shouldBuffer && isExecutionPhase, just accumulate content silently
                }
              } catch (parseError) {
          ...
          // Process tool calls into actions with error recovery (ORIGINAL LOGIC WITH ALL ERROR HANDLING)
          let actions = null;
          if (toolCalls.length > 0) {
            actions = toolCalls.map((toolCall, index) => {
              try {
                ...
                // Parse arguments (ORIGINAL ERROR RECOVERY LOGIC)
                try {
                  ...
                  // Apply all the original error recovery logic here
                  // ... (keeping all the original regex repair logic from lines 711-907)
                  ...
                  // Step 2: Apply all original fixes
                  fixedArgs = fixedArgs
                    .replace(/^{\s*"?title"?\s*:\s*([^"][^",}]*)/i, '{"title":"$1"')
                    .replace(/^{\s*"?title"?\s*:\s*"?([^"]*)/i, '{"title":"$1"');
                  ...
                  try {
                    ...
                    // Final fallback - use minimal args
                    console.error(`All recovery methods failed for tool call ${index}`);
                    return null;
                  }
                }

                // Map function name to action type (ORIGINAL LOGIC)
                const actionTypeMap = {
          ...
          // Generate a title if this is the first message (ORIGINAL LOGIC)
          let title = null;
          if (isFirstMessage) {
            title = await generateTitle(userMessage, messageHistory);
          }

          // Process actions to include action links and enhanced responses (ORIGINAL LOGIC)
          ...
          if (actions && actions.length > 0) {
            ...
            // Enhanced response to mention the modal functionality (ORIGINAL LOGIC)
            ...
            // Add action links (ORIGINAL LOGIC)
          ...
          // Send complete response (ORIGINAL LOGIC)
        ...
        // Handle the streamResponse action (ORIGINAL LOGIC)
        await sendToClient(connectionId, {
          type: 'status',
          status: 'acknowledged',
...
// Helper function to send messages to the client (ORIGINAL)
...
// Process action data to ensure it matches expected format by frontend (ORIGINAL WITH ALL DOMAIN MAPPING)
  ...
  switch (functionName) {
    case 'createGoal':
      return {
        ...
        domain: args.domain,
        color: getDomainColor(args.domain), // Uses domain mapping, ignores AI's color
        icon: getDomainIcon(args.domain),   // Uses domain mapping, ignores AI's icon
    ...
    case 'createMilestone':
      ...
      const tasks = Array.isArray(args.tasks)
        ? args.tasks.map((task, index) => {
            // Handle both string tasks and object tasks from AI
            if (typeof task === 'string') {
            ...
            } else if (task && typeof task === 'object' && task.title) {
              return {
              ...
              return {
                id: `task_${Date.now()}_${index}_${Math.random().toString(36).substr(2, 9)}`,
                title: String(task), // Fallback to string conversion
                status: 'todo',
                completed: false
              };
            }
      ...
      return {
        ...
        domain: args.domain,
        color: getDomainColor(args.domain), // Uses domain mapping
        tasks: tasks,
    ...
    case 'createTimeBlock':
      ...
      // Helper function to fix Claude's inconsistent date formatting (ORIGINAL LOGIC)
...
// Helper function to get domain color based on domain name (ORIGINAL)
...
// Helper function to get domain icon based on domain name (ORIGINAL)
...
// Function to generate an intelligent conversation title using AI (ORIGINAL)
...
// Improved fallback function (ORIGINAL)
